// ABOUTME: Excel file upload page for importing scores from spreadsheets.
// ABOUTME: Parses .xlsx/.xls files with SheetJS and shows preview before import.
...
export function ExcelUploadPage({ leagueId, matchId }: ExcelUploadPageProps) {
  ...
  const applyParseResult = useCallback((result: ExcelParseResult) => {
  ...
  const handleFileChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      reader.onload = (event) => {
        ...
        setFileData(data);
        const result = parseExcelScores(data);
  ...
  const handleSheetChange = useCallback(
    (e: React.ChangeEvent<HTMLSelectElement>) => {
      ...
      if (fileData) {
        const result = parseExcelScores(fileData, sheet);
        applyParseResult(result);
      }
    },
    [fileData, applyParseResult],
  );
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <h2 className="text-2xl font-bold mb-6">Import Excel Scores</h2>
      ...
      {sheetNames.length > 1 && (
        <div className="mb-6">
          <label htmlFor="sheet-select" className="block text-sm font-medium mb-2">
            Select sheet
          </label>
          <select
            id="sheet-select"
